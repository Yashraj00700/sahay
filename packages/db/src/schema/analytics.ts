...
export const analyticsDaily = pgTable(
  "analytics_daily",
  {
    ...
    codConversionRevenue: decimal("cod_conversion_revenue", {
      precision: 12,
      scale: 2,
    }).default("0"),
    upsellRevenue: decimal("upsell_revenue", {
      precision: 12,
      scale: 2,
    }).default("0"),
    // Messages
  ...
  (table) => ({
    tenantDateIdx: uniqueIndex("idx_analytics_tenant_date_channel").on(
      table.tenantId,
      table.date,
      table.channel,
    ),
...
export const waTemplates = pgTable(
  ...
  (table) => ({
    tenantNameLangIdx: uniqueIndex("idx_wa_templates_tenant_name_lang").on(
      table.tenantId,
      table.name,
      table.languageCode,
    ),
...
export const cannedResponses = pgTable(
  ...
  (table) => ({
    ...
    shortcutIdx: index("idx_canned_shortcut").on(
      table.tenantId,
      table.shortcut,
    ),
  }),
