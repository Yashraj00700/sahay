...
async function processWhatsAppPost(
      ...
      for (const msg of value.messages ?? []) {
        sends.push(
          sendEvent({
            ...
            data: {
              tenantId: tenant.id,
              raw: { ...msg, _phoneNumberId: phoneNumberId } as Record<string, unknown>,
            },
          }),
        )
      }
      ...
      for (const status of value.statuses ?? []) {
        sends.push(
          sendEvent({
            ...
            data: {
              tenantId: tenant.id,
              raw: { ...status, _phoneNumberId: phoneNumberId } as Record<string, unknown>,
            },
          }),
        )
      }
  ...
  for (const r of results) {
    if (r.status === 'rejected') {
      log.error({ reason: r.reason }, 'Failed to enqueue WhatsApp Inngest event')
    }
  }
}
