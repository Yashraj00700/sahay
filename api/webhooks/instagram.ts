...
async function processInstagramPost(
    ...
    for (const ev of entry.messaging ?? []) {
      ...
      const hasMessage =
        ...
        (typeof ev.message.text === "string" ||
          (Array.isArray(ev.message.attachments) &&
            ev.message.attachments.length > 0));
      const hasPostback = ev.postback != null;

      if (!hasMessage && !hasPostback) {
        log.debug(
          { ev },
          "Instagram messaging event with no message/postback, skipping",
        );
        continue;
      }
  ...
  for (const r of results) {
    if (r.status === "rejected") {
      log.error(
        { reason: r.reason },
        "Failed to enqueue Instagram Inngest event",
      );
    }
  }
}
