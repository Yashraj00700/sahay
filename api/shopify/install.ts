...
export default async function handler(
  ...
  try {
    ...
    res
      .status(429)
      .json({ error: "Too many install attempts. Try again shortly." });
    return;
  }
  ...
  const value: InstallStateValue = {
    shop,
    createdAt: new Date().toISOString(),
  };

  try {
    await upstash.set(stateKey(state), value, {
  ...
  log.info(
    { shop, requestId: req.headers["x-request-id"] },
    "redirecting to Shopify authorize",
  );
