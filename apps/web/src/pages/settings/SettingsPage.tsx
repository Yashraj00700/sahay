...
function GeneralTab() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await api.patch("/settings/brand", form);
    } catch {
      /* use local state */
    }
    setSaved(true);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 max-w-xl">
      ...
      <Field
        label="Support email"
        hint="Shown in notifications to agents and customers"
      >
        <Input
          type="email"
      ...
      <Field
        ...
        hint="Outside these hours, conversations queue for the next day"
      >
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-text-secondary mt-0.5">
              Automatically close with no recent activity
            </p>
          </div>
          <Toggle
        ...
        {form.autoClose && (
          <Field label="Close after (hours)">
            <Input
              ...
              onChange={(e) => set("autoCloseHours", parseInt(e.target.value))}
              className="w-24"
            />
          </Field>
        )}
...
function AISettingsTab() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await api.patch("/settings/ai", form);
    } catch {
      /* local only */
    }
    setSaved(true);
    ...
    <form onSubmit={handleSubmit} className="space-y-5 max-w-xl">
      <div className="p-4 bg-surface border border-border rounded-xl flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-text-secondary mt-0.5">
            Allow AI to send responses without human review
          </p>
        </div>
        <Toggle
        ...
        <>
          ...
          >
            <input
              ...
              min={50}
              max={99}
              value={form.autoRespondThreshold}
              onChange={(e) =>
                set("autoRespondThreshold", parseInt(e.target.value))
              }
              className="w-full accent-primary"
            />
            <div className="flex justify-between text-xs text-text-secondary mt-1">
              <span>50% — more auto</span>
              <span>99% — more human review</span>
            </div>
          </Field>
          ...
          <Field
            label="Max AI responses per conversation"
            hint="Prevents loops — after this many AI messages, route to human"
          >
            <Input
              type="number"
              min={1}
              max={20}
              value={form.maxAutoResponses}
              onChange={(e) =>
                set("maxAutoResponses", parseInt(e.target.value))
              }
              className="w-24"
            />
          </Field>
          ...
          <div className="p-4 bg-surface border border-border rounded-xl flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-text-secondary mt-0.5">
                Route to senior agent when customer is upset
              </p>
            </div>
            <Toggle
              checked={form.escalateOnSentiment}
              onChange={(v) => set("escalateOnSentiment", v)}
            />
          </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span
                className={cn(
                  ...
                  ROLE_BADGE[m.role] ?? "bg-border text-text-secondary",
                )}
              >
                {m.role}
              </span>
      ...
      fields: [
        {
          key: "widgetId",
          label: "Widget ID",
          value: "wgt_ras_prod_abc123",
          secret: false,
        },
        {
          key: "domains",
          label: "Allowed domains",
          value: "rasluxuryoils.com",
          secret: false,
        },
      ],
    },
      ...
      fields: [
        {
          key: "shopDomain",
          label: "Shop domain",
          value: "ras-luxury-oils.myshopify.com",
          secret: false,
        },
        {
          key: "shopToken",
              ...
              {ch.fields.map((f) => {
                ...
                return (
                  <div key={f.key} className="flex items-center gap-3">
                    <label className="text-xs text-text-secondary w-44 flex-shrink-0">
                      {f.label}
                    </label>
                    <div className="flex-1 relative">
                      <Input
                        type={
                          f.secret && !showTokens[showKey] ? "password" : "text"
                        }
...
function NotificationsTab() {
  ...
  if (!push.supported) {
    ...
    statusText =
      "Notifications are blocked. Update site settings in your browser to re-enable.";
    statusTone = "warn";
  } else if (push.subscribed) {
    statusText =
      "You will receive a push notification when a conversation needs attention.";
    statusTone = "ok";
  } else {
    statusText =
      "Get notified about new conversations and mentions even when Sahay is in the background.";
    statusTone = "off";
  }
    ...
    <div className="h-full flex flex-col bg-background">
      <div className="px-6 pt-6 pb-0 border-b border-border flex-shrink-0">
        ...
        <p className="text-sm text-text-secondary mt-0.5 mb-4">
          Configure your Sahay workspace
        </p>
