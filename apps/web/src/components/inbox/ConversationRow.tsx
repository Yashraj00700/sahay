...
function leftBorderColor(conv: Conversation): string {
  if (conv.csatScore !== undefined && conv.csatScore < 3)
...
function TierBadge({ tier }: { tier: string }) {
  if (tier === "vip")
    return (
      <span className="text-[10px]" title="VIP">
        👑
      </span>
    );
  if (tier === "loyal")
    return (
      <span className="text-[10px]" title="Loyal">
        ⭐
      </span>
    );
  return (
...
export function ConversationRow({
  ...
  const agentsViewing = useInboxStore(
    (s) => s.agentsViewing[conversation.id] ?? [],
  );

  const customerName = conversation.customer?.name ?? "Unknown";
  const avatarCls = useMemo(
    () => deterministicPastel(customerName),
    [customerName],
  );
  const borderCls = leftBorderColor(conversation);
  const sentiment =
    SENTIMENT_MAP[conversation.sentiment] ?? SENTIMENT_MAP.neutral;
  ...
  return (
    <motion.div
      ...
      className={clsx(
        ...
        borderCls,
        isActive ? "bg-violet-50 border-l-violet-500" : "hover:bg-violet-50/60",
      )}
    >
      {/* Avatar */}
