...
export function CustomerSidebar({
  ...
  const LTV_CAP = 50000;
  const ltvProgress = Math.min(
    ((customer?.totalSpent ?? 0) / LTV_CAP) * 100,
    100,
  );
  ...
  const addTag = useCallback(
    (tag: string) => {
      ...
      if (trimmed && !localTags.includes(trimmed)) {
        ...
        api.patch(`/conversations/${conversation.id}/tags`, {
          tags: [...localTags, trimmed],
        });
      }
      setTagInput("");
    },
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                <span className="text-[11px]">
                  {LANG_FLAGS[customer?.languagePref ?? "auto"]}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2.5">
                ...
                {latestOrder.items.slice(0, 4).map((item, i) => (
                  ...
                  >
                    {item.imageUrl ? (
                      ...
                      <span className="text-[8px] text-gray-400 text-center px-0.5 leading-tight">
                        {item.name}
                      </span>
                    )}
                  </div>
            ...
            {latestOrder.estimatedDelivery && (
              <div className="text-[11px] text-gray-500">
                Est. delivery:{" "}
                {new Date(latestOrder.estimatedDelivery).toLocaleDateString(
                  "en-IN",
        ...
        <div className="space-y-3">
          ...
          {customer?.churnRisk && (
            <div className="flex items-center justify-between">
              ...
              >
                {customer.churnRisk.charAt(0).toUpperCase() +
                  customer.churnRisk.slice(1)}
              </span>
            </div>
          )}
