...
export function CommandPalette({
  isOpen: isOpenProp,
  onClose: onCloseProp,
  onSelectConversation: onSelectProp,
}: CommandPaletteProps) {
  ...
  const { data: searchData } = useQuery<SearchResponse>({
    ...
    queryFn: async () => {
      const res = await api.get<SearchResponse>("/search/suggest", {
        params: { q: query },
      });
      return res.data;
    },
  ...
  const conversations: MessageSearchResult[] =
    searchData?.results.conversations ?? [];
  ...
  const filteredActions =
    query.length > 0
      ? quickActions.filter(
          (a) => fuzzyScore(a.label + " " + a.description, query) > 0,
        )
      ...
      if (e.key === "ArrowDown") {
        ...
        if (item.type === "conversation") {
          ...
          onSelectConversation(
            (item.data as MessageSearchResult).conversationId,
          );
          onClose();
          ...
          >
                  ...
                  {customers.map((cust) => {
                    ...
                    return (
                      <ResultItem
                        ...
                        secondary={[cust.email, cust.city]
                          .filter(Boolean)
                          .join(" · ")}
                        badge={
                          cust.tier !== "new" ? (
                            <span className="text-[10px]">
                              {cust.tier === "vip" ? "VIP" : "LOYAL"}
                            </span>
                          ) : undefined
                        }
                        onClick={() => {
            ...
            <div className="flex items-center gap-4 px-4 py-2 border-t border-gray-100 bg-gray-50/60">
              <div className="flex items-center gap-1">
                <kbd className="text-[9px] text-gray-400 bg-white border border-gray-200 px-1 py-0.5 rounded font-mono">
                  ↑↓
                </kbd>
