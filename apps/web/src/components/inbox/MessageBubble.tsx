...
function VoiceNotePlayer({ durationSeconds }: { durationSeconds?: number }) {
  ...
  const bars = Array.from({ length: barCount }, (_, i) => {
    const h =
      4 + Math.sin(i * 0.9) * 6 + Math.cos(i * 1.7) * 4 + Math.random() * 4;
        ...
        {bars.map((h, i) => (
          <div
            ...
            className={clsx(
              ...
              playing && i < barCount * 0.4 ? "bg-violet-600" : "bg-gray-300",
            )}
          />
        ))}
      </div>
...
function ConfidenceBadge({ confidence }: { confidence?: number }) {
  ...
  return (
    <span
      className={clsx("text-[9px] px-1.5 py-0.5 rounded-full font-medium", cls)}
    >
      {pct}%
    </span>
  );
