...
function defaultMimeForType(type: FetchAndStoreInstagramMediaArgs['attachmentType']): string {
  switch (type) {
    case 'image':
      return 'image/jpeg'
    case 'video':
...
export async function fetchAndStoreInstagramMedia(
  ...
  try {
    ...
    throw new IntegrationError('instagram_media', 'payloadUrl is not a valid URL')
  }
