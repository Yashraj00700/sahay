...
export function decideRouting(params: RoutingParams): RoutingResult {
  ...
  if (stopKeyword) {
    return {
      ...
      reason:
        "STOP keyword detected — consent revocation requires immediate human handling",
      urgency: 5,
      notifyNow: true,
    };
  }
  ...
  if (allergyCheck || intent === "allergy_check") {
    return {
      decision: "route_to_human",
      reason:
        "Ingredient allergy query — safety concern requires human response",
      urgency: 4,
      notifyNow: true,
    };
  }
  ...
  if (sentiment === "very_negative") {
    ...
    if (customerTier === "vip") {
      return {
        decision: "route_to_senior",
        reason:
          "VIP customer with very negative sentiment — senior agent escalation",
        urgency: 5,
        notifyNow: true,
      };
    }
  ...
  if (
    ...
    return {
      decision: "draft_for_review",
      reason:
        "VIP customer with complaint — draft response for agent review before sending",
      urgency: 3,
      notifyNow: false,
    };
  }
