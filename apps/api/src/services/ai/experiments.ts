...
async function loadActiveExperiment(
  ...
  const live = rows.filter((r: { startsAt: Date | null; endsAt: Date | null }) => {
  ...
  const tenantScoped = live.find((r: { tenantId: string | null }) => r.tenantId !== null)
...
export async function getVariant(
  ...
  const runner = async (
    ...
    if (existingRows[0]) {
      const matched = exp.variants.find((v) => v.name === existingRows[0].variant)
      if (!matched) {
    ...
    const inserted = await conn
      ...
      .returning({ id: experimentAssignments.id, variant: experimentAssignments.variant })
...
export function normaliseVariants(
  ...
  if (!Array.isArray(raw)) return { ok: false, reason: 'variants must be an array' }
  const out: ExperimentVariant[] = []
  let totalWeight = 0
  const seen = new Set<string>()
  for (const item of raw) {
    if (!item || typeof item !== 'object') return { ok: false, reason: 'variant must be an object' }
    const v = item as Record<string, unknown>
    const name = typeof v.name === 'string' ? v.name.trim() : ''
    if (!name) return { ok: false, reason: 'variant.name is required' }
    if (seen.has(name)) return { ok: false, reason: `duplicate variant name: ${name}` }
    seen.add(name)
    const weight = typeof v.weight === 'number' ? v.weight : Number(v.weight)
    if (!Number.isFinite(weight) || weight < 0) {
      return { ok: false, reason: `variant.weight must be a non-negative number (got ${String(v.weight)})` }
    }
    totalWeight += weight
    const config =
  ...
  if (totalWeight <= 0) return { ok: false, reason: 'sum of variant weights must be > 0' }
