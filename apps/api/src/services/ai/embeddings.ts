...
export async function generateEmbedding(text: string): Promise<number[]> {
  ...
  if (!embedding)
    throw new Error("[embeddings] OpenAI returned empty embedding");
...
export async function generateBatchEmbeddings(
  ...
  if (uncachedTexts.length > 0) {
    ...
    await Promise.all(
      uncachedTexts.map(async (text, j) => {
        ...
        if (!embedding)
          throw new Error(`[embeddings] Missing embedding at index ${j}`);
  ...
  return results.map((r, i) => {
    if (r === null)
      throw new Error(`[embeddings] Missing result for index ${i}`);
    return r;
  });
}
