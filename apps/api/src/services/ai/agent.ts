...
function scrubMedicalClaims(text: string): {
  text: string;
...
async function fetchShopifyOrder(
  ...
  try {
    ...
    const fulfillments =
      (o.fulfillments as Array<Record<string, unknown>> | undefined) ?? [];
    const trackingNumbers = fulfillments[0]?.tracking_numbers as
      | string[]
      | undefined;
    ...
    return {
      ...
      lineItems: ((o.line_items as Array<Record<string, unknown>>) ?? []).map(
        (li) => ({
...
function buildSystemPrompt(ctx: SystemPromptContext): string {
  ...
  const prohibited =
    ctx.prohibitedPhrases.length > 0
      ? `\n## NEVER use these phrases\n${ctx.prohibitedPhrases.map((p) => `- "${p}"`).join("\n")}`
      : "";

  const preferred =
    ctx.preferredPhrases.length > 0
...
export async function runAIPipeline(
  ...
  if (!lastCustomerMsg?.content) {
    throw new Error(
      `[agent] No customer message in conversation ${conversationId}`,
    );
  }
  ...
  if (ORDER_INTENTS.has(intentResult.intent)) {
    const orderEntity = intentResult.entities.find(
      (e) => e.type === "order_id",
    );
  ...
  const skinTypeEntity = intentResult.entities.find(
    (e) => e.type === "skin_type",
  );
  ...
  const experimentOverrides:
    | SystemPromptContext["experimentOverrides"]
    | undefined = experimentSelection
    ? {
        ...
        examples:
          typeof expCfg.examples === "string" ? expCfg.examples : undefined,
        extraInstructions:
  ...
  try {
    ...
    if (block.type !== "text")
      throw new Error("[agent] Unexpected Claude response block type");
  ...
  if (scrubResult.hadViolation) {
    console.warn(
      `[agent] Medical claim scrubbed — conversation ${conversationId}`,
    );
  }
  ...
  if (containsPII(responseText)) {
    console.error(
      `[agent] PII in AI response — routing to human (conversation ${conversationId})`,
    );
    ...
    return {
      ...
      citations: [],
      escalationReason:
        "PII detected in AI response — routed to human for safety",
      processingMs: Date.now() - startTime,
    };
  }
  ...
  const [insertedMsg] = await db
    ...
    .values({
      ...
      isAiDraft,
      aiConfidence: String(
        parsedResponse.confidence ?? intentResult.confidence,
      ),
