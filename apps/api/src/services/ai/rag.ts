...
async function vectorSearch(
  ...
  if (filters?.productId)
    filterParts.push(`kc.product_id = '${filters.productId}'`);
  if (filters?.category)
    filterParts.push(`kc.category = '${filters.category}'`);
  if (filters?.chunkType)
    filterParts.push(`kc.chunk_type = '${filters.chunkType}'`);
  if (filters?.language)
    filterParts.push(`kc.language = '${filters.language}'`);
...
async function bm25Search(
  ...
  if (filters?.skinType) {
    filterParts.push(
      `(kc.skin_types IS NULL OR kc.skin_types = '{}' OR '${filters.skinType}' = ANY(kc.skin_types))`,
    );
  }
...
function rerank(
  chunks: FullChunkRow[],
  rrfEntries: Map<string, RRFEntry>,
): Array<
  FullChunkRow & {
    finalScore: number;
    vectorSimilarity?: number;
    ...
    .map((chunk) => {
      ...
      if (chunk.lastUpdated) {
        ...
        if (ageDays < RECENCY_MAX_DAYS) {
          const recencyBonus =
            RECENCY_WEIGHT * (1 - ageDays / RECENCY_MAX_DAYS);
          score += recencyBonus;
        }
      }
