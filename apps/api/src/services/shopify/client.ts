 ...
 */
function shouldThrottle(header: string | null): {
  throttle: boolean;
  ratio: number;
} {
...
export class ShopifyClient {
  ...
  readonly rest: RestNamespace = {
    ...
    put: <T>(path: string, body: unknown) =>
      this.request<T>(path, { method: "PUT", body }),
  ...
  private async request<T>(path: string, init: RequestInit): Promise<T> {
    ...
    while (attempt < maxAttempts) {
      ...
      try {
        ...
        if (throttle) {
          logger.debug(
            { shop: this.shop, ratio },
            "shopify near rate limit; sleeping 500ms",
          );
          await sleep(500);
        }
        ...
        if (
          ...
          !(err.message.includes(" 429 ") || /\b5\d\d\b/.test(err.message))
        ) {
          throw err;
        }
        lastErr = err;
        ...
        const backoff = 250 * 2 ** (attempt - 1);
        logger.warn(
          { shop: this.shop, attempt, backoff, err },
          "shopify transport retrying",
        );
        await sleep(backoff);
      }
    }
