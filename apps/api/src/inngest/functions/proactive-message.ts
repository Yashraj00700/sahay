  ...
  async ({ event, step, logger }) => {
    ...
    const customer = await step.run("reload-customer", async () =>
      withTenant(tenantId, async (tx) => {
        ...
        if (!row)
          throw new Error(
            `proactive-message: customer ${customerId} not found`,
          );
        return {
          id: row.id,
    ...
    if (customer.tenantId !== tenantId) {
      logger.warn(
        { customerId, tenantId },
        "proactive-message: tenant mismatch — skip",
      );
    ...
    if (customer.isOptout) {
      logger.info(
        { customerId },
        "proactive-message: customer opted out — skip",
      );
    ...
    if (!customer.whatsappId) {
      logger.info(
        { customerId },
        "proactive-message: no whatsappId on customer — skip",
      );
