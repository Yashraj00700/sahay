...
export const webchatIncoming = inngest.createFunction(
  ...
  async ({ event, step, logger }) => {
    ...
    if (!sessionId || !message) {
      logger.warn(
        { tenantId },
        "webchat-incoming: missing sessionId or message body",
      );
      return { skipped: true };
    }
    ...
    const customer = await step.run("upsert-customer", async () =>
      withTenant(tenantId, async (tx) => {
        ...
        if (!created)
          throw new Error("webchat-incoming: failed to insert customer");
      ...
      withTenant(tenantId, async (tx) => {
        ...
        if (!existing || sessionExpired) {
          const [created] = await tx
            ...
            .returning();
          if (!created)
            throw new Error("webchat-incoming: failed to insert conversation");
    ...
    const storedMessage = await step.run("insert-message", async () =>
      withTenant(tenantId, async (tx) => {
        ...
        if (!created)
          throw new Error("webchat-incoming: failed to insert message");
