  ...
  async ({ event, step, logger }) => {
    ...
    try {
      ...
      logger.error(
        ...
        "ai-respond: pipeline failure — falling back to human",
      );
    ...
    if (result.decision === "auto_respond" && result.responseText) {
      await step.run("dispatch-outgoing", async () => {
        ...
        const { conv, customer } = await withTenant(tenantId, async (tx) => {
          ...
          if (!conv)
            throw new Error("ai-respond: conversation not found for dispatch");
          const customer = await tx.query.customers.findFirst({
            where: eq(customers.id, conv.customerId),
          });
          if (!customer)
            throw new Error("ai-respond: customer not found for dispatch");
