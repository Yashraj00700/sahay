  ...
  async ({ event, step, logger }) => {
    ...
    if (resource === "orders") {
      return step.run("sync-orders", async () => {
        ...
        logger.info(
          { tenantId, count: list.length },
          "shopify-sync: orders synced",
        );
  ...
  await withTenant(tenantId, async (tx) => {
    ...
    const values = {
      ...
      subtotalPrice: raw["subtotal_price"]
        ? String(raw["subtotal_price"])
        : null,
      totalTax: raw["total_tax"] ? String(raw["total_tax"]) : null,
      totalDiscounts: raw["total_discounts"]
        ? String(raw["total_discounts"])
        : null,
      lineItemCount: Array.isArray(raw["line_items"])
        ? (raw["line_items"] as unknown[]).length
        : null,
      ...
      cancelledAt: raw["cancelled_at"]
        ? new Date(String(raw["cancelled_at"]))
        : null,
