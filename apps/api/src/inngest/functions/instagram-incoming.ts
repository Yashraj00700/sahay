...
function parseInstagramEntry(raw: Record<string, unknown>): ParsedIGMessage {
  ...
  if ((raw as { sender?: unknown }).sender !== undefined) {
    ...
    const entry = (raw as { entry?: Array<Record<string, unknown>> })
      .entry?.[0];
    const messagingArr = (
      entry as { messaging?: Array<Record<string, unknown>> } | undefined
    )?.messaging;
  ...
  const message =
    (messaging["message"] as Record<string, unknown> | undefined) ?? {};
  const messageId = String(message["mid"] ?? "");
  const timestamp = String(
    messaging["timestamp"] ?? Math.floor(Date.now() / 1000),
  );
...
export const instagramIncoming = inngest.createFunction(
  ...
  async ({ event, step, logger }) => {
    ...
    const parsed = await step.run("parse", async () =>
      parseInstagramEntry(raw),
    );

    if (!parsed.igUserId || !parsed.messageId) {
      logger.warn(
        { tenantId },
        "instagram-incoming: skipping payload with no sender/mid",
      );
      return { skipped: true };
    }
    ...
    const customer = await step.run("upsert-customer", async () =>
      withTenant(tenantId, async (tx) => {
        ...
        if (!created)
          throw new Error("instagram-incoming: failed to insert customer");
    ...
    const conversation = await step.run("upsert-conversation", async () =>
      withTenant(tenantId, async (tx) => {
        ...
        if (!existing || sessionExpired) {
          ...
          if (!created)
            throw new Error(
              "instagram-incoming: failed to insert conversation",
            );
    ...
    const storedMessage = await step.run("insert-message", async () =>
      withTenant(tenantId, async (tx) => {
        ...
        if (!created)
          throw new Error("instagram-incoming: failed to insert message");
